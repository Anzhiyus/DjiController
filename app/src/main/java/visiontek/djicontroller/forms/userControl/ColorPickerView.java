...
public class ColorPickerView extends View {
    ...
    public void setProgressVal(int val){
        ...
        if (orientation == Orientation.HORIZONTAL) { // 水平
            y = (rect.bottom - rect.top) / 2+mRadius-5;//不知道为什么减 总之不减位置不对
            float interval=(float)_max/(rect.right-rect.left);
            x=_max-val;
            if(x==0){x=1;}
            if(x==_max){x=_max-1;}
            x=(int)(x/interval+rect.left);
        } else { // 竖直
            x = (rect.right - rect.left) / 2+mRadius-5;//不知道为什么减 总之不减位置不对
            float interval=(float) _max/(rect.bottom-rect.top);
            y=_max-val;
            if(y==0){y=1;}
            if(y==_max){y=_max-1;}
            y = (int)(y/interval+rect.top);
        }
        setPosition(x,y);
    }
    public int getProgressVal(){//保证不超出max和min范围
        int x, y,res;
        if (orientation == Orientation.HORIZONTAL) { // 水平
        ...
        if(Math.abs(res-_max)<5){
            res=_max;
        }
        if(Math.abs(res-_min)<5){
            res=_min;
        }
        return  res;
    }
